...
def init_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="normal_only",
        help="Only generate normal (benign) queries, skipping the sqlmap attack and "
        "insider-threat stages. Produces a fresh benign-only pool (all rows labelled 0, "
        "split='train') drawn from the same templates/dictionaries as the main datasets, "
        "with novel random fills. Used to mint the extra benign samples that double the "
        "'big' train sets.",
    )
    ...
    parser.add_argument(
        ...
        dest="normal_count",
        default=400_000,
        help="Number of normal samples to generate per dataset in --normal-only mode.",
    )
...
def main():
    ...
    for dataset_config in datasets:
        ...
        if args.ithreat_only:
            ...
            db.build_normal_only(args, args.normal_count)
        else:
            db.build(args)
