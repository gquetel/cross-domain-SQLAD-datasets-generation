...
class iThreatGenerator:
    ...
    def perform_insider_attack_sqlmap(self):
        ...
        # Each objective carries the ATT&CK technique it realizes, declared next to
        # the sqlmap flag it runs so the label cannot drift out of sync with it.
        # --schema enumerates structure (discovery); the others read/extract stored
        # values (exfiltration).
        if self.testing_mode:
            objectives = [("--schema", attack_labels.T1082)]
        else:
            objectives = [
                ("--dump", attack_labels.T1020),  # Dump DBMS database table entries
                ("--passwords", attack_labels.T1020),  # Enumerate users password hashes
                ("--schema", attack_labels.T1082),  # Enumerate DBMS schema / structure
                ("--all", attack_labels.T1020),  # Retrieve everything
            ]

        df_res = pd.DataFrame()
        cnt_obj = 0

        for obj, technique in objectives:
            ...
            _df = pd.DataFrame(
                {
                    ...
                    attack_labels.ATTACK_LABEL_COLUMN: technique,
                    "split": "test",
                }
            )
    ...
    def perform_insider_attack_metasploit(self):
        ...
        # Other scenarios does not fit with insider threats. Each carries the ATT&CK
        # technique it realizes: schemadump enumerates structure (discovery),
        # hashdump extracts stored credentials (exfiltration).
        scenarios = [
            (sc_schemadump, attack_labels.T1082),
            (sc_userdump, attack_labels.T1020),
        ]
        # Quiet, command mode
        base_command = "msfconsole -q -x "

        for sc, technique in scenarios:
