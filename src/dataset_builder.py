...
class DatasetBuilder:
        ...
        if conditions_mask.any():
            for idx in self.df_atk_templates[conditions_mask].index:
                ...
                if table_name and self.condition_generator.has_conditions(table_name):
                    ...
                    logger.debug(
                        f"Instantiated {{conditions}} for attacks in {template_id}: {new_template}"
                    )
    ...
    def build_normal_only(self, args, n_normal: int):
        """Generate a benign-only pool of novel normal queries (no attack stage).

        Reuses the exact normal-generation path of ``build`` (template selection in
        ``populate_normal_templates`` followed by dictionary filling in
        ``generate_normal_queries``), so the produced rows share the benign
        distribution of the main datasets but carry fresh random fills. Used to mint
        the extra benign samples needed to double the train sets of the "big"
        datasets, without re-running the expensive sqlmap attack stage.

        Note: for datasets that ship SQL-script statements (e.g. OHR), those are
        reused verbatim; only the template-supplemented portion is novel.

        Args:
            args: Parsed CLI arguments (uses ``testing`` and ``no_syn_check``).
            n_normal: Number of normal samples to generate.
        """
        do_syn_check = not args.no_syn_check

        self.df = pd.DataFrame()
        self.init_templates(testing_mode=args.testing)
        self.populate_normal_templates(n_normal)
        self.generate_normal_queries(do_syn_check)

        # All rows are benign and destined to feed train pools.
        self.df["split"] = "train"
        self._remove_user_input_admin()

    def _add_attack_labels(self):
        """Ensure every row carries an ``ATT&CK label`` technique before saving.

        Insider rows arrive already tagged by ``iThreatGenerator`` (each objective
        declares its own technique, see :mod:`attack_labels`), because all insiders
        share ``attack_stage == "exploit"`` and so cannot be told apart by stage.
        Here we fill in the rest: SQLIA attacks are labeled from their
        ``attack_stage`` and benign queries get an empty label.
        """
        df = self.df
        col = attack_labels.ATTACK_LABEL_COLUMN

        # Start from any label the generators already set (insiders); default empty.
        if col in df.columns:
            labels = df[col].fillna("")
        else:
            labels = pd.Series("", index=df.index, dtype="object")

        # SQLIA attacks: derive from stage. Benign rows (label 0) and already-tagged
        # insider rows (statement_type == "insider") are left untouched. A benign-only
        # dataset may not carry these columns at all.
        if "attack_stage" in df.columns:
            statement_type = df.get("statement_type", pd.Series("", index=df.index))
            is_sqlia = (df["label"] == 1) & (statement_type != "insider")
            labels.loc[is_sqlia] = df.loc[is_sqlia, "attack_stage"].map(
                attack_labels.technique_for_stage
            )

        df[col] = labels.fillna("")

    def save(self, output_dir: str):
        os.makedirs(output_dir, exist_ok=True)
        outpath = os.path.join(output_dir, f"{self.dataset_name}.csv")

        self._add_attack_labels()
