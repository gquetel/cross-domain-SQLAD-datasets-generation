...
REPO_ROOT = "~/datasets/superviz26-full"
FULL_OUTPUT_DIR = os.path.expanduser("~/datasets/superviz26-lodo")
...
def count_splits(path, chunksize=500_000):
    """Return {split_value: row_count} for a dataset, via a single chunked pass."""
    counts = {}
    for chunk in pd.read_csv(path, chunksize=chunksize, usecols=["split"], low_memory=False):
...
def main():
    ...
    parser.add_argument(
        "--full",
        action="store_true",
        help="Use the full datasets with no fixed subsampling: train/test sizes are set "
        "to the smallest domain's train/test pool so all scenarios stay equal-sized. "
        "Writes to ~/datasets/superviz26-lodo by default.",
    )
    args = parser.parse_args()

    if args.full and (args.tiny or args.concept_drift):
        parser.error("--full cannot be combined with --tiny or --concept-drift")

    output_dir = args.output_dir or (FULL_OUTPUT_DIR if args.full else SCRIPT_DIR)
    os.makedirs(output_dir, exist_ok=True)

    names = list(DATASETS.keys())

    if args.full:
        print("=== Computing full-mode sizes (downsampling to smallest pool) ===")
        pool = {n: count_splits(DATASETS[n]) for n in names}
        train_size = min(c.get("train", 0) for c in pool.values())
        test_size = min(c.get("test", 0) for c in pool.values())
        suffix = ""
        for n in names:
            print(f"  {n}: train={pool[n].get('train', 0)}, test={pool[n].get('test', 0)}")
        print(f"  -> full train_size={train_size}, test_size={test_size}")
    elif args.tiny:
        train_size, test_size, suffix = TINY_TRAIN_SIZE, TINY_TEST_SIZE, "-tiny"
    else:
        train_size, test_size, suffix = TRAIN_SIZE, TEST_SIZE, ""
    ...
    if args.concept_drift:
        ...
        for name in targets:
            ...
            out_origin = os.path.join(output_dir, f"origin-{name}{suffix}.csv")
            ...
            out_shifted = os.path.join(output_dir, f"shifted-{name}{suffix}.csv")
        ...
        if args.in_domain_only:
            ...
            for target in targets:
                ...
                outpath = os.path.join(
                    output_dir, f"{train_letters}-{target_letter}{suffix}.csv"
                )
        ...
        for target in targets:
            ...
            outpath = os.path.join(output_dir, f"{letter}-{letter}{suffix}.csv")
            out.to_csv(outpath, index=False)
